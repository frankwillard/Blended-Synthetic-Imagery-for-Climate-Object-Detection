...
def blend_with_gp_gan(
    ...
    assert list_path or (
        src_img and dst_img and mask_img
    ), "Either list_path or src_img, dst_img, and mask_img must be specified"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Creates a synthetic image by placing random objects and blending them with background images blending images through the GP-GAN"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="File path to the GP-GAN generator model",
    )
    parser.add_argument(
        "--src-img",
        type=str,
        required=True,
        help="File path for source image to blend into destination image",
    )
    parser.add_argument(
        "--dst-img",
        type=str,
        required=True,
        help="File path of destination image to blend source image into",
    )
    parser.add_argument(
        "--mask-img",
        type=str,
        required=True,
        help="File path of mask image with information as to where to blend source image into destination image",
    )
    parser.add_argument(
        "--blended-img-out-path",
        type=str,
        default=None,
        help="File path of mask image with information as to where to blend source image into destination image",
    )
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print out progress of blending"
    )
    args = parser.parse_args()

    assert not (
        args.list_path and (args.src_img or args.dst_img or args.mask_img)
    ), "Either list_path or src_img, dst_img, and mask_img must be specified, not both. Otherise list_path will just be used"
