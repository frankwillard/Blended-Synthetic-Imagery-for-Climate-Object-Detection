...
def main():
    ...
    parser.add_argument(
        "--ngf", type=int, default=64, help="# of base filters in decoder or G"
    )
    parser.add_argument(
        "--nc", type=int, default=3, help="# of output channels in decoder or G"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_iteration", type=int, default=1000, help="# of iterations for optimizing z"
    )
    ...
    parser.add_argument(
        "--g_path",
        default="/scratch/public/GP-GAN/models/blending_gan.npz",
        help="Path for pretrained Blending GAN model",
    )
    parser.add_argument(
        "--unsupervised_path",
        default="/scratch/public/GP-GAN/models/unsupervised_blending_gan.npz",
    ...
    parser.add_argument(
        "--blended_image", default="", help="Where to save blended image"
    )
    ...
    if args.supervised:
        G = EncoderDecoder(
            args.nef, args.ngf, args.nc, args.nBottleneck, image_size=args.image_size
        )
        ...
        print(
            "Load pretrained unsupervised Blending GAN model from {} ...".format(
                args.unsupervised_path
            )
        )
