...
def main():
    ...
    parser.add_argument(
        "--workers", type=int, default=10, help="# of data loading workers"
    )
    ...
    parser.add_argument(
        "--snapshot_interval", type=int, default=1, help="Interval of snapshot (epoch)"
    )
    parser.add_argument(
        "--print_interval",
        type=int,
    ...
    trainset = H5pyDataset(
        args.data_root, load_size=args.load_size, crop_size=args.image_size
    )
    ...
    noisev = Variable(
        np.asarray(
            np.random.normal(size=(args.test_size, args.nz, 1, 1)), dtype=np.float32
        )
    )
    noisev.to_gpu(args.gpu)
    trainer.extend(
        sampler(G, path, noisev, "fake_samples_{}.png"), trigger=plot_interval
    )
