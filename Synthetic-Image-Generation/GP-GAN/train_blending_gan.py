...
def main():
    ...
    parser.add_argument(
        "--nef", type=int, default=64, help="# of base filters in encoder"
    )
    parser.add_argument(
        "--ngf", type=int, default=64, help="# of base filters in decoder"
    )
    parser.add_argument(
        "--nc", type=int, default=3, help="# of output channels in decoder"
    )
    parser.add_argument(
        "--nBottleneck", type=int, default=4000, help="# of output channels in encoder"
    )
    ...
    parser.add_argument(
        "--beta1", type=float, default=0.5, help="Beta for Adam, default=0.5"
    )
    parser.add_argument(
        "--l2_weight",
        type=float,
    ...
    parser.add_argument(
        "--n_epoch", type=int, default=25, help="# of epochs to train for"
    )

    parser.add_argument("--data_root", help="Path to dataset")
    parser.add_argument(
        "--load_size", type=int, default=64, help="Scale image to load_size"
    )
    parser.add_argument(
        "--image_size",
        type=int,
    ...
    parser.add_argument(
        "--d_iters", type=int, default=5, help="# of D iters per each G iter"
    )
    parser.add_argument(
        "--clamp_lower", type=float, default=-0.01, help="Lower bound for clipping"
    )
    parser.add_argument(
        "--clamp_upper", type=float, default=0.01, help="Upper bound for clipping"
    )
    ...
    parser.add_argument(
        "--test_folder", default="samples", help="Where to store test results"
    )
    parser.add_argument(
        "--workers", type=int, default=4, help="# of data loading workers"
    )
    parser.add_argument("--batch_size", type=int, default=64, help="Input batch size")
    parser.add_argument(
        "--test_size", type=int, default=64, help="Batch size for testing"
    )

    parser.add_argument(
        "--train_samples", type=int, default=150000, help="# of training examples"
    )
    parser.add_argument(
        "--test_samples", type=int, default=256, help="# of testing examples"
    )
    ...
    parser.add_argument(
        "--snapshot_interval", type=int, default=1, help="Interval of snapshot (epochs)"
    )
    parser.add_argument(
        "--print_interval",
        type=int,
    ...
    parser.add_argument(
        "--plot_interval", type=int, default=10, help="Interval of plot (iteration)"
    )
    ...
    train_iter = chainer.iterators.MultiprocessIterator(
        trainset, args.batch_size, n_processes=args.workers, n_prefetch=args.workers
    )
    ########################################################################################################################
    ...
    trainer.extend(
        extensions.snapshot_object(G, "g_epoch_{.updater.epoch}.npz"),
        trigger=snapshot_interval,
    )
    trainer.extend(
        extensions.snapshot_object(D, "d_epoch_{.updater.epoch}.npz"),
        trigger=snapshot_interval,
    )
    ...
    trainer.extend(
        extensions.PrintReport(["iteration", "main/loss", "D/loss", "main/l2_loss"]),
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/loss"], "iteration", file_name="loss.png", trigger=plot_interval
        )
    )
    trainer.extend(
        extensions.PlotReport(
            ["D/loss"], "iteration", file_name="d_loss.png", trigger=plot_interval
        )
    )
    trainer.extend(
        extensions.PlotReport(
    ...
    trainer.extend(
        sampler(G, path, train_v, "fake_samples_train_{}.png"), trigger=plot_interval
    )

    val_batch = [valset[idx][0] for idx in range(args.test_size)]
    val_v = Variable(chainer.dataset.concat_examples(val_batch, args.gpu))
    trainer.extend(
        sampler(G, path, val_v, "fake_samples_val_{}.png"), trigger=plot_interval
    )
